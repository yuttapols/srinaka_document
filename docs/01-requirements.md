...
ระบบเว็บร้านสปา/นวด ให้ลูกค้าดูสินค้า/บริการ+ราคา, สมัครสมาชิก (กรอกเอง/LINE) + ยืนยันตัวตนก่อนถึงจะ
...
| **Customer** | ลูกค้า ดูสินค้า/บริการ+ราคาได้โดยไม่ login แต่**ต้องสมัครสมาชิก+ยืนยันตัวตนผ่าน LINE ก่อนถึงจะจองได้** (ไม่มี guest booking) | ดู public content ไม่ต้อง login, จองต้อง login+verify |
...
- FR-1.7 Customer สมัครสมาชิกได้ 2 ทาง — **กรอกเอง** (username/password), **LINE** — ทุกทางบังคับ
...
- FR-1.8 บัญชีที่สมัครผ่าน **LINE ถือว่า verified ทันที** (`verified_at` = เวลาสมัคร) เพราะ OAuth
  คือการยืนยันตัวตนอยู่แล้ว
- FR-1.9 บัญชีที่สมัคร**กรอกเอง**ยังไม่ verified — ต้อง login เข้าระบบก่อน แล้วไป**ลิงก์บัญชี LINE
  เพิ่ม** ผ่านหน้า profile ระบบจะผูก `line_user_id` เข้ากับ user เดิม
  (ไม่ merge บัญชีใหม่ ไม่สร้าง user ซ้ำ) แล้ว set `verified_at` ทันที
  (**Google login ถูกพิจารณาไว้แล้วแต่ตัดออก** — ติดขั้นตอน verify billing ของ Google Cloud ที่ยุ่งยาก/มี
  ค่าใช้จ่ายล่วงหน้าโดยไม่จำเป็น ใช้ LINE เพียงอย่างเดียวพอ — ถ้าจะเพิ่ม Google ทีหลังก็ทำได้โดยไม่กระทบ
  โครงสร้างเดิม)
...
- FR-2.2 แก้ไขชื่อ, เบอร์โทร ได้อิสระ — การแก้เบอร์โทรไม่กระทบสถานะ `verified_at` เลย (verify มาจากการลิงก์
  LINE ไม่ใช่จากตัวเบอร์โทรอีกต่อไป)
